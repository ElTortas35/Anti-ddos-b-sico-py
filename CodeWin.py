...
def detectar_inundacion_syn(paquete):
    ...
    if paquete.haslayer(TCP) and paquete[TCP].flags == 'S':
        ...
        if conteo_syn[ip_origen] > umbral:
            ...
            #implementan el bloqueo de la IP usando netsh
            
            os.system(f"netsh advfirewall firewall add rule name='Bloquear_{ip_origen}' dir=in interface=any action=block remoteip={ip_origen}")
